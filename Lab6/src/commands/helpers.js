...
const crtLoop = (disk, keys, count = 0, user = 'admin', type = 'directory', content = '', isSecure = false) => {
    ...
    return {
        ...disk, files: {
            ...disk.files, [keys[count]]: crtLoop(disk.files[keys[count]], keys, count + 1, user, type, content, isSecure
            )
        }
    }
}

const createDisk = (disk, currentDir, file = '', user = 'admin', type = 'directory', content = '', isSecure = false) => {
    const fileNames = (currentDir + file).split('/').filter(Boolean);
